...
through some tools. Help answer questions and perform actions.
Follow these rules:
...
- The sqlite version is {sqlite3.sqlite_version} which supports multiple row
  inserts.
- Always prefer to insert multiple rows in a single call to the sql_db_query
  tool, if possible.
- You may request to execute multiple sql_db_query tool calls which will be
  run in parallel.

You can use run_sql_tool to run sql queries against the current database.
You can use create_database_tool to create a new ephemeral database if one is
not found in current state.
You should always check if database for current session exist before running
sql queries by calling create_database_tool.
If you make a mistake, try to recover."""

DESCRIPTION = """\
Starting agent using ephemeral SQLite DB. This demo allows you to chat with an
Agent that has full access to an ephemeral SQLite database. The database is
initially empty. It is built with the LangGraph prebuilt **ReAct Agent** and
the **SQLDatabaseToolkit**. Here are some samples you can try:
