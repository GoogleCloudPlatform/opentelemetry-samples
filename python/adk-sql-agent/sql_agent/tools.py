...
class SqlRunResult(TypedDict):
    ...
    """If the result represents an error. It will be null or undefined if no
    error"""
...
def create_database_tool(tool_context: ToolContext) -> dict[str, Any]:
    ...
    """
    if SESSION_DB_KEY not in tool_context.state:
        _, path = tempfile.mkstemp(suffix=".db")
        # No scope prefix in the state data indicates that it will be persisted
        # for current session.
    ...
    db_path = tool_context.state[SESSION_DB_KEY]
    return {"resp": f"Skipping database creation, {db_path} already exists"}


def run_sql_tool(sql_query: str, tool_context: ToolContext) -> dict[str, Any]:
    """Runs a SQLite query. The SQL query can be DDL or DML.
    Returns the rows if it's a SELECT query."""
    ...
    with sqlite3.connect(current_session_db_path) as db:
        try:
            ...
            # commit here ensures DML changes are saved.
            db.commit()
            ...
            try:
                ...
                # This can happen if the connection is already closed or in an
                # unusable state.
